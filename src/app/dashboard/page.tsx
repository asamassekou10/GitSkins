...
export default function DashboardPage() {
  ...
  if (status === 'loading') {
    return (
      <div style={{ minHeight: '100vh', background: 'linear-gradient(180deg, #0a0a0a 0%, #111 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
    ...
    <div style={{ minHeight: '100vh', background: 'linear-gradient(180deg, #0a0a0a 0%, #111 100%)', color: '#e5e5e5' }}>
      <div style={{ maxWidth: '1000px', margin: '0 auto', padding: '100px 20px 60px' }}>
        ...
        <div style={{ marginBottom: '40px' }}>
          ...
          <p style={{ color: '#888' }}>Manage your account, subscription, and usage</p>
        </div>
        ...
        <div style={{ ...cardStyle, marginBottom: '24px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '20px', flexWrap: 'wrap' }}>
            <img
              src={avatarUrl}
              alt={displayName}
              style={{ width: 80, height: 80, borderRadius: '50%', border: isPro ? '3px solid #22c55e' : '3px solid #2a2a2a' }}
            />
            <div style={{ flex: 1, minWidth: '200px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'wrap' }}>
                <h2 style={{ fontSize: '24px', fontWeight: 600, color: '#fff', margin: 0 }}>{displayName}</h2>
                {isPro && (
                  <span style={{ padding: '4px 12px', background: 'linear-gradient(135deg, rgba(34, 197, 94, 0.2) 0%, rgba(74, 222, 128, 0.2) 100%)', border: '1px solid #22c55e', borderRadius: '20px', color: '#22c55e', fontSize: '12px', fontWeight: 600 }}>
                    PRO
                  </span>
                )}
              </div>
            ...
            <Link
              ...
              style={{ padding: '10px 20px', background: 'transparent', border: '1px solid #2a2a2a', borderRadius: '10px', color: '#888', fontSize: '14px', fontWeight: 500, textDecoration: 'none', transition: 'all 0.2s' }}
          ...
          <div style={cardStyle}>
            ...
            {isPro ? (
              <button
                ...
                style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', padding: '8px 16px', background: 'transparent', border: '1px solid #2a2a2a', borderRadius: '8px', color: '#888', fontSize: '13px', fontWeight: 600, cursor: portalLoading ? 'not-allowed' : 'pointer' }}
              >
              ...
              <Link
                href="/pricing"
                style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', padding: '8px 16px', background: 'rgba(34, 197, 94, 0.1)', border: '1px solid rgba(34, 197, 94, 0.3)', borderRadius: '8px', color: '#22c55e', fontSize: '13px', fontWeight: 600, textDecoration: 'none' }}
              >
                Upgrade to Pro →
              </Link>
            )}
        ...
        <div style={cardStyle}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '12px' }}>
            {[
              ...
              { href: `/showcase/${user.username}`, label: 'View Showcase', sub: 'Your profile widgets', icon: <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#22c55e" strokeWidth="2"><rect x="3" y="3" width="18" height="18" rx="2" /><circle cx="8.5" cy="8.5" r="1.5" /><polyline points="21 15 16 10 5 21" /></svg> },
              { href: '/#themes', label: 'Browse Themes', sub: `Explore all ${isPro ? '20' : FREE_THEMES.length} themes`, icon: <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#22c55e" strokeWidth="2"><circle cx="12" cy="12" r="10" /><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z" /><path d="M2 12h20" /></svg> },
              { href: '/ai', label: 'AI Features', sub: 'Profile analysis & more', icon: <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#22c55e" strokeWidth="2"><path d="M12 2a10 10 0 1 0 10 10H12V2z" /><path d="M12 2a10 10 0 0 1 10 10" /><circle cx="12" cy="12" r="6" /></svg> },
            ].map((item) => (
              <Link
                key={item.href}
                href={item.href}
                style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '16px', background: '#1a1a1a', border: '1px solid #2a2a2a', borderRadius: '12px', color: '#fff', textDecoration: 'none', transition: 'all 0.2s' }}
